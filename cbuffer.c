...
void __cbuf_put(cbuf_t* buf, int index, char value, uint8_t type) {
  ...
  nlog(MSG_LOG, "cbuf_put", "set index %d to value %p with type %c\n", index, value, type == CBUF_DATA ? 'D' : 'F');
}
...
char __cbuf_get(cbuf_t* buf, int index, uint8_t* type) {
  ...
  *type = (uint8_t)buf->data[__cbuf_type(cbuf_mod(buf, index))];
  ...
  nlog(MSG_LOG, "cbuf_get", "data at index %d has value %p and type %c\n", index, data, *type == CBUF_DATA ? 'D' : 'F');
