...
#define cbuf_loop(c,start,len,i,v,t)            do { int __cbuf_c=0,  __cbuf_l=1; uint8_t __cbuf_t; \
                                                for(i=cbuf_mod(c,start),v=__cbuf_get(c,i,&t,1),__cbuf_t=t;\
                                                    __cbuf_c < len && (__cbuf_t == t) && __cbuf_l; \
                                                    cbuf_inc(c,i),v=__cbuf_get(c,i,&__cbuf_t,1),__cbuf_l=__cbuf_t==CBUF_DATA,__cbuf_c++)
...
#define cbuf_r_loop(c,start,len,i,v)            do { int __cbuf_c = 0; uint8_t  __cbuf_t; \
                                                for(i=cbuf_mod(c,start),v=__cbuf_get(c,i,&__cbuf_t,1);\
                                                    __cbuf_c < len; \
                                                    cbuf_inc(c,i),v=__cbuf_get(c,i,&__cbuf_t,1),__cbuf_c++)
