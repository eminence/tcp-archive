  ...
  while(1) {
	 for(fd=0; fd < MAXSOCKETS; fd++) {
		...
		if (sendFlagNext(sock)) {
		  ...
		  nlog(MSG_LOG, "tcp_send_thread", "It appears the next thing to send is a flag(%p), calling send_packet_with_flags() (seq %d -> %d)", flags, sock->send_next, sock->send_next+1);

		  //send_packet_with_flags(sock, flags, 0);
		  tcp_sendto(sock, NULL, 0, flags); /* Can't use send packet with flags; SKIP QUEUE! */
		  sock->send_next++;   //XXX  increase SEQUENCE NUMBER by one

		} else if ((amount = getAmountAbleToSend(sock)) > 0) {
		  /* we have regular data to send */
		  nlog(MSG_LOG,"tcp_send_thread", "There are %d bytes waiting to be sent on socket %d (seq before: %d)", amount, fd, sock->send_next);
