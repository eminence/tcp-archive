...
tcp_machine_t* tcpm_new(tcp_socket_t* context, uint8_t clone) {
  ...
  assert(0 == state_transition(st_estab,       st_close_wait,  ON_RECV_FIN,        do_send_flags,          alloc_flags(TCP_FLAG_ACK))); /* XXX XXX do_recv_tcp DOESN'T sends ack anymore. XXX XXX*/
